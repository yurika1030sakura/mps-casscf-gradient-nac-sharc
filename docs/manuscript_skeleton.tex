...
limit at high $M$.  The CAS(8,8) C$_2$ system is retained as a larger
active-space root-tracking benchmark.  LiF provides a small-gap,
nonzero-derivative-coupling benchmark; in the smallest basis, the scalar
single-point NAC error is more sensitive to state-gauge rotations than the
corresponding energy and gradient errors.
...
matrix.  Each entry is the base-10 logarithm of the largest-$M$ gradient
error in m$\Eh$/Bohr or NAC error in atomic units.}
\label{fig:highm}
\end{figure}
