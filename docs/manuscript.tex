...
\title{Analytic State-Averaged DMRG-CASSCF Gradients and Nonadiabatic Couplings for SHARC-Compatible PySCF/pyblock2 Workflows}
...
FCI-optimized orbitals.  An MPS-Krylov response backend stores active-space
response vectors as MPS objects and is exercised in fixed-orbital
FCI-free response endpoints, including an anthracene $\pi$-space
CAS(14,14) benchmark whose post hoc FCI-scored strict-response errors are
$1.57\times 10^{-3}$ m$\Eh$/Bohr for gradients and
$9.11\times 10^{-6}$ a.u. for the derivative coupling.  Short SHARC
trajectory smoke tests on public H$_2$ and ethylene models demonstrate that
the DMRG-CASSCF backend can be called repeatedly by SHARC and writes the
Hamiltonian, dipole, gradient, derivative-coupling, and phase data blocks
requested during NACDR dynamics.
\end{abstract}
...
\midrule
\input{largest_m_table_rows.tex}
\bottomrule
\end{tabular}
\end{table}
...
\subsection{DMRG-SHARC Trajectory Smoke Tests}
...
mode.  Each trajectory completed three SHARC frames, corresponding to steps
...
dipoles, gradients, derivative couplings, and phase bookkeeping.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Short public DMRG-CASSCF SHARC trajectory smoke tests.}
...
FCI-optimized orbitals.  The MPS-Krylov response backend and fixed-orbital
MPS-only SHARC facade provide a runtime path that does not store the
active-space response vector as a dense determinant array; in a CAS(14,14)
anthracene endpoint it reproduces FCI-scored gradients to
$1.57\times10^{-3}$ m$\Eh$/Bohr and derivative couplings to
$9.11\times10^{-6}$ a.u. with no runtime FCI root selection.  Short
DMRG-SHARC trajectory smoke tests verify that the same backend can be called
repeatedly by SHARC in NACDR mode.  The
results provide a reproducible software platform for testing DMRG-based
multireference response quantities and for connecting them to nonadiabatic
dynamics interfaces.
