...
\end{align}
The orbital Schur system has the dimension of the orbital-rotation space (four
variables for the HeH$^+$ example below) and is solved with a dense iterative
solver, each application of the Schur operator requiring one
$\mathbf{H}_{CC}^{-1}$ evaluation.  Every $\mathbf{H}_{CC}^{-1}$ application is a
...
with a coupled-system residual of $2\times10^{-15}$).
