...
def _gradient_one_state_mps_krylov(
    ...
    If ``precomputed_z=(kappa, lci_mps)`` is supplied, the internal response solve
    is skipped and the gradient is assembled directly from that z-vector (e.g. a
    response solved by the sweep-localized Schur backend).  This lets us assemble
    the physical gradient g and compare g.q to FD even when the response carries a
    loose true-residual -- the gradient contraction is far less sensitive to the
    high-rank CI tail than the full residual.
    """
    from pyscf.grad import sacasscf as sacasscf_grad
