...
# Step 7 add-on: analytic CP-CASSCF gradient + NAC. Imported lazily-safe at
# module load (no heavy work happens until `compute_grad_nac_analytic_cp` is
# called from run_calc).
try:
    from analytic_cp_sharc import (
        compute_grad_nac_analytic_cp,
        fill_grad_nac_arrays,
    )
    _ANALYTIC_CP_AVAILABLE = True
...
at least one task"""
    ...
    #     analytic-nac true
    template_dict["analytic-nac"] = False
    # Full DMRG-CASSCF SHARC currently still projects DMRG roots to PySCF FCI
    ...
    for line in template:
        ...
        if key.startswith("spin"):
            ...
            template_dict[key] = orig[1].strip()
    ...
    ALLOWED_METHODS = ["casscf", "l-pdft", "mc-pdft", "cms-pdft", "dmrg-hybrid", "dmrg-casscf"]
...
def gen_solver(mol, qmin):
    ...
    if len(qmin["states"]) != 1:
        ...
        if qmin["method"] == 0 or qmin["method"] == 4 or qmin["method"] == 5:
        ...
        if qmin["method"] == 5:
            ...
            n_dets = (
                int(cistring.num_strings(int(ncas), neleca))
            ...
            solver.fcisolver = MPSAsFCISolver(
                mol,
                bond_dim=int(qmin["template"].get("dmrg-maxm", 500)),
                n_sweeps=int(qmin["template"].get("dmrg-nsteps", 30)),
                n_threads=max(1, int(qmin.get("ncpu", 1))),
                sweep_tol=float(qmin["template"].get("dmrg-sweep-tol", 1.0e-7)),
            ...
            solver.fcisolver.fix_spin_(
                ss=0.0,
                shift=float(qmin["template"]["fix-spin-shift"]),
            )
            print(
        ...
        elif qmin["method"] == 2 or qmin["method"] == 0 or qmin["method"] == 4 or qmin["method"] == 5:
    ...
    if qmin["method"] == 4:
        ...
        print(
            "Warning: dmrg-hybrid uses SA-CASSCF gradients/NACs and an experimental "
            "DMRG overlay for energies/dipoles. Treat results as exploratory.",
            flush=True,
        )
...
def get_dipole_elements(solver):
    if hasattr(solver, "get_dipole_elements"):
        return solver.get_dipole_elements()
...
def run_calc(qmin):
    ...
    use_analytic_cp = bool(qmin.get("template", {}).get("analytic-nac", False))
    if use_analytic_cp and not _ANALYTIC_CP_AVAILABLE:
        print(
            "[SHARC_PYSCF_ext] WARNING: analytic-nac requested but "
            f"analytic_cp_sharc import failed ({_ANALYTIC_CP_IMPORT_ERR}). "
            "Falling back to default gradient/NAC path.",
            flush=True,
        )
        use_analytic_cp = False

    if use_analytic_cp and (qmin["gradmap"] or qmin["nacmap"]):
        # Build 0-based state lists for our CP solver
        grad_zb = sorted({s - 1 for (mult, s) in qmin["gradmap"]}) \
            if qmin["gradmap"] else []
        nac_zb = []
        if qmin["nacmap"]:
            for (m1, s1, m2, s2) in qmin["nacmap"]:
                nac_zb.append((s1 - 1, s2 - 1))

        # The mc object we feed CP-CASSCF must be the underlying CASSCF.
        # If solver is a hybrid wrapper, fetch its underlying mc.
        mc_for_cp = solver
        ...
        if qmin["gradmap"]:
            result["grad"] = grad_list
        if qmin["nacmap"]:
            result["nacdr"] = nac_arr
        err += e
    else:
        if qmin["gradmap"]:
