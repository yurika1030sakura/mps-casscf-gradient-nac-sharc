...
class ResponseBudgetExceeded(RuntimeError):
    """Raised when a response solve exceeds its wall-clock budget.

    Carries the best-available residual so the driver can record an honest
    certificate-flagged (non-converged) result instead of hanging. This is the
    graceful-degradation path for near-degenerate / ill-conditioned systems where
    the response is genuinely singular (small sigma_min) and no iterative solve
    will converge within any reasonable budget.
    """
...
class CPDMRGCASSCFResponseMPSKrylov(CPDMRGCASSCFResponseMPS):
    ...
    def __init__(self, mc, driver, mpo_active, *, mps_states=None,
        ...
        # Robustness guards for near-degenerate / ill-conditioned response systems
        # (e.g. FCI-accessible near-degenerate polyenes, or geometries approaching a
        # conical intersection in dynamics). A singular response makes the Krylov
        # residual stagnate; without a guard the solver runs to max_iter accumulating
        # untruncated Krylov vectors and the downstream Lagrangian assembly then grinds
        # for hours on the bloated multiplier MPS. The stagnation guard breaks early
        # when the residual stops improving; the optional wall budget is a hard ceiling.
        # Both are conservative: a well-conditioned solve converges (hits tol) long
        # before the stagnation window elapses, so normal runs are unaffected.
        self._stagnation_window = 15    # consecutive iters with < rtol improvement -> stop
        self._stagnation_rtol = 1.0e-2  # "improvement" = residual drop by >= 1%
        self._stagnation_min_iter = 8   # never stagnation-break before this many iters
        self._wall_budget_s = None      # optional hard wall-clock ceiling for a solve
    ...
    def gmres_mps(
        ...
        *,
        ...
        # stagnation tracking: a singular (near-degenerate) response plateaus the
        # residual; break early rather than accumulate untruncated Krylov vectors.
        stag_best = beta
        stag_since = 0
        stagnated = False
        deadline = (solve_t0 + float(self._wall_budget_s)
                    if self._wall_budget_s else None)

        for j in range(max_iter):
            if deadline is not None and time.perf_counter() > deadline:
                raise ResponseBudgetExceeded(
                    f"MPS-GMRES wall budget {self._wall_budget_s:.0f}s exceeded at "
                    f"iter {j} (residual {residual:.3e})",
                    residual=float(residual), niter=j,
                )
            t0 = time.perf_counter()
            ...
            # stagnation guard: track best residual seen; if it has not improved by
            # at least stagnation_rtol over stagnation_window consecutive iterations,
            # the response is effectively singular and further iterations only bloat
            # the solution MPS -> stop now with the honest (non-converged) residual.
            if residual < stag_best * (1.0 - self._stagnation_rtol):
                stag_best = residual
                stag_since = 0
            else:
                stag_since += 1
            if (self._stagnation_window
                    and j + 1 >= self._stagnation_min_iter
                    and stag_since >= self._stagnation_window):
                stagnated = True
                if self._verbose_solver:
                    print(f"  MPS-GMRES stagnation: residual flat at {residual:.3e} "
                          f"for {stag_since} iters -> stop at iter {j + 1}", flush=True)
                break
    ...
    def cr_mps(
        ...
        *,
        ...
        for it in range(1, max_iter + 1):
            if deadline is not None and time.perf_counter() > deadline:
                raise ResponseBudgetExceeded(
                    f"MPS-CR wall budget {self._wall_budget_s:.0f}s exceeded at "
                    f"iter {it} (residual {residual:.3e})",
                    residual=float(residual), niter=it,
                )
            ...
            # plateaus the residual, rather than iterate to max_iter.
            if residual < stag_best * (1.0 - self._stagnation_rtol):
                stag_best = residual
                stag_since = 0
            else:
                stag_since += 1
            if (self._stagnation_window
                    and it >= self._stagnation_min_iter
                    and stag_since >= self._stagnation_window):
                stagnated = True
                if self._verbose_solver:
                    print(f"  MPS-CR stagnation: residual flat at {residual:.3e} "
                          f"for {stag_since} iters -> stop at iter {it}", flush=True)
                r = r_next
                break
