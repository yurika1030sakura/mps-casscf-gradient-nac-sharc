...
def _apply_orbital_sign_flip(driver, mps, orbital, *, ncas, tag,
                             bond_dim=None):
    """Apply the exact spatial-orbital gauge flip ``a_p^+ -> -a_p^+``.

    On a Fock state this is the local parity operator ``(-1)**n_p``.  Written
    as a real number-conserving polynomial it is

        1 - 2 n_p + 4 n_{p alpha} n_{p beta}.

    The corresponding SU(2)-adapted QC MPO is applied to the MPS.  This
    supplies the disconnected O(n) component that cannot be generated by a
    real antisymmetric orbital-rotation logarithm.  ``add_ident=True`` is
    required by block2's sweep application machinery; it adds the internal
    identity channel, not another physical identity term.
    """
    orbital = int(orbital)
    if not 0 <= orbital < int(ncas):
        raise IndexError(f"orbital sign-flip index {orbital} outside ncas={ncas}")
    h1e = np.zeros((ncas, ncas))
    g2e = np.zeros((ncas, ncas, ncas, ncas))
    h1e[orbital, orbital] = -2.0
    g2e[orbital, orbital, orbital, orbital] = 4.0
    parity = driver.get_qc_mpo(
        h1e=h1e, g2e=g2e, ecore=1.0,
        symmetrize=True, add_ident=True, iprint=0,
    )
    out = driver.copy_mps(mps, tag)
    bd = int(bond_dim) if bond_dim is not None else int(mps.info.bond_dim)
    driver.multiply(
        out, parity, mps, n_sweeps=4, tol=1.0e-12,
        bra_bond_dims=[bd], iprint=0,
    )
    return out
...
def rotate_mps_orbitals(driver, mps, s, *, ncas, tag, n_steps=24,
    ...
    smat = np.asarray(s, dtype=float)
    U, P = polar(smat)                            # s = U @ P

    ket = driver.copy_mps(mps, f"{tag}-0")
    reflection_orbital = None
    if float(np.linalg.det(U)) < 0.0:
        # O(n) has two disconnected components.  Move a det=-1 polar factor
        # into SO(n) by a single column sign flip D, and apply the corresponding
        # exact parity transformation to the MPS.  Choose the diagonal element
        # that maximises trace(U D), i.e. keeps the residual rotation smallest.
        p = int(np.argmin(np.diag(U)))
        D = np.eye(ncas)
        D[p, p] = -1.0
        reflection_orbital = p
        smat = smat @ D
        U, P = polar(smat)
        if float(np.linalg.det(U)) < 0.0:
            raise RuntimeError("discrete orbital sign surgery did not repair det=-1")
    ...
    if float(np.linalg.norm(Xg)) > 1.0e-14:
        ket = _apply_onebody_exp(driver, ket, Xg, ncas=ncas, tag=f"{tag}-U",
                                 sign=conv["u_sign"], hermitian=False,
                                 n_steps=n_steps, bond_dim=bond_dim)
    if reflection_orbital is not None:
        # ``rotate_mps_orbitals`` is calibrated in the ket-orbital convention,
        # whose second-quantized composition order is reversed relative to the
        # matrix product above.  Apply the disconnected parity component after
        # the determinant-positive rotation.  Root phases may change; those are
        # legitimate electronic gauges and are fixed by the overlap graph.
        ket = _apply_orbital_sign_flip(
            driver, ket, reflection_orbital, ncas=ncas,
            tag=f"{tag}-REFLECT", bond_dim=bond_dim,
        )
    return ket
...
def cross_geometry_raw_overlap_matrix(obj_ref, obj_disp, mol_ref, mol_disp,
                                      mo_ref, mo_disp, ncore, ncas, nst, *,
                                      host_frame, tag,
                                      include_stretch=False):
    """Unmodified ``<Psi_I(ref)|Psi_J(disp)>`` matrix -- FCI-free.

    This is the production boundary for an immutable overlap-graph artifact:
    columns remain in the target endpoint's stored root order and no electronic
    state phase is changed.  Root permutation/sign choices belong to the gauge
    compiler and must never be hidden inside a record labelled ``raw``.

    ``include_stretch=False`` applies the best-fit unitary polar transport used
    by the validated finite-difference NAC route.  If the non-unitary active-
    space stretch is requested, the caller must record that fact and validate
    the stretch integrator independently before treating the result as a
    literal cross-basis many-body overlap.

    Returns ``(O_raw, s, diagnostics)``.  The diagnostics bind the transport
    convention and expose active-subspace leakage instead of silently folding
    it into state tracking.
    """
    import block2
    ...
    for j in range(nst):
        ...
        rot = rotate_mps_orbitals(
            host_drv, loaded, s.T, ncas=ncas, tag=f"{tag}rot{j}",
            include_stretch=bool(include_stretch),
        )
        O_raw[:, j] = np.array([
            obj_ref._mps_overlap(obj_ref._state_mps[i], rot)
            for i in range(nst)
        ])

    svals = np.linalg.svd(s, compute_uv=False)
    diagnostics = {
        "schema": "mps-native-raw-overlap-diagnostics-v1",
        "direction": "<source/reference raw roots | target/displaced raw roots>",
        "root_assignment_applied": False,
        "root_phase_fix_applied": False,
        "orbital_transport": {
            "rotation_convention": dict(ROTATION_CONVENTION),
            "include_nonunitary_stretch": bool(include_stretch),
            "active_cross_overlap_determinant": float(np.linalg.det(s)),
            "active_cross_overlap_singular_values": svals.tolist(),
            "active_cross_overlap_condition_number": float(
                np.inf if np.min(svals) == 0.0
                else np.max(svals) / np.min(svals)
            ),
            "active_subspace_leakage_max": float(
                np.max(np.abs(1.0 - svals))
            ),
        },
        "raw_root_overlap_singular_values": np.linalg.svd(
            O_raw, compute_uv=False,
        ).tolist(),
    }
    return O_raw, s, diagnostics


def cross_geometry_overlap_matrix(obj_ref, obj_disp, mol_ref, mol_disp,
                                  mo_ref, mo_disp, ncore, ncas, nst, *,
                                  host_frame, tag):
    """Continuously tracked ``<Psi_I(ref)|Psi_J(disp)>`` -- FCI-free.
    ...
    """
    O_raw, s, _diagnostics = cross_geometry_raw_overlap_matrix(
        obj_ref, obj_disp, mol_ref, mol_disp, mo_ref, mo_disp,
        ncore, ncas, nst, host_frame=host_frame, tag=tag,
        include_stretch=False,
    )
