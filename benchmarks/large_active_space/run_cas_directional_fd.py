...
Large-CAS directional-gradient validation.  Instead of arbitrary Cartesian components,
...
def build_progressive(symbols, coords_bohr, basis, ncas, nelecas, *,
    ...
    # CI-vector pinning: reuse a persistent MPS scratch across builds so each
    # displaced SA-CASSCF reloads the previous geometry's converged MPS as its
    # initial guess (the trajectory's on-branch mechanism) -- prevents the
    # CI-level basin lottery that the orbital-only warm-start cannot.
    if mps_persistent_dir is not None:
        cfg0["mps_persistent_dir"] = str(mps_persistent_dir)
...
def run(n_carbon, *, basis="sto-3g", m_schedule=None, threads=8,
        ...
        response_m_compress=None, response_solver=None, response_tol=1.0e-7):
    ...
    try:
        # Moderate response bond dim so the analytic response COMPLETES at
        # beyond-FCI sizes (with the m_compress-cap the cost is bounded; the
        # true residual stays loose but the GRADIENT contraction g.q is far less
        # sensitive than the full residual -- this measures the physical
        # analytic-vs-FD agreement, which is what the validation needs).
        if response_m_compress is not None:
            mc0.fcisolver.response_m_compress = int(response_m_compress)
        if response_solver is not None:
            mc0.fcisolver.response_linear_solver = str(response_solver)
        res = compute_grad_nac_analytic_cp(mc0, gradient_states=[0], nac_pairs=None,
                                           backend="mps-krylov", tol=float(response_tol),
                                           max_iter=300)
...
def main():
    ...
    ap.add_argument("--response-m-compress", type=int, default=None,
                    help="cap the analytic response bond dim so it completes at beyond-FCI")
    ap.add_argument("--response-solver", default=None, help="gmres|cr|bicgstab")
    ap.add_argument("--response-tol", type=float, default=1.0e-7)
    ...
    try:
        r = run(args.ncarbon, basis=args.basis, m_schedule=sched, threads=args.threads,
                ...
                response_m_compress=args.response_m_compress,
                response_solver=args.response_solver, response_tol=args.response_tol)
